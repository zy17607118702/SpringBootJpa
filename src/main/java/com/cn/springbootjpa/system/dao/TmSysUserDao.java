...
public interface TmSysUserDao extends BaseDao<TmSysUser, Integer> {
	@Query(value = "select * from tm_sys_user t where t.system_name=?1 and t.system_pwd=?2", nativeQuery = true)
	public TmSysUser checkPassword(String userName, String password);

	@Query(value = "select * from tm_sys_user t where t.system_name=?1", nativeQuery = true)
