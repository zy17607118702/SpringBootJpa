...
public class UpdateTimeUtil {
	...
	public static void main(String[] args) {
		...
		UpdateTimeUtil.addAuditableEntityClass("com.saicmotor.imes.model.TmSysUser");
	 ...
	 * @return hql
	 */
	public static String addUpdateFieldToHql(String hql, Map<String, Object> params) {
		String returnStr = "";
		String aliasName = "";
		// 判断是否是updateHQL
		if (!isUpdateHQL(hql)) {
		...
		if (isUpdateObjectInstanceOfDataAuditEntityBase(updateObjectName)) {
			...
			if (list != null) {
				...
				if (aliasName != null && !"".equals(aliasName)) {
					list.add(setWordOffset, aliasName + "." + BaseEntity.FIELD_UPDATETIME + "=:LastUpdateTime,");
					params.put("LastUpdateTime", new Date());

					list.add(setWordOffset,
							aliasName + "." + BaseEntity.FIELD_UPDATEBY + "=:LastUpdateUsername,");
					params.put("LastUpdateUsername", LoginUserUtil.getLoginUser());
				} else {
					list.add(setWordOffset, BaseEntity.FIELD_UPDATETIME + "=:LastUpdateTime,");
					params.put("LastUpdateTime", new Date());
					list.add(setWordOffset, BaseEntity.FIELD_UPDATEBY + "=:LastUpdateUsername,");
					params.put("LastUpdateUsername", LoginUserUtil.getLoginUser());
	 ...
	 * @return 1 or 0
	 */
	private static boolean isUpdateObjectInstanceOfDataAuditEntityBase(String updateObjectName) {
		try {
	...
	public static void addAuditableEntityClass(Class clazz) {
		...
		if (ClassUtils.isAssignable(BaseEntity.class, clazz))
	...
	public static void addAuditableEntityClass(String className) {
		try {
			addAuditableEntityClass(ClassUtils.forName(className, null));
		} catch (Exception e) {
			log.error(e);
		}
	}
